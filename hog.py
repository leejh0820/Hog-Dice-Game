...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        current = score0 if who == 0 else score1
        gain = current - last_score
        
        if gain > running_high:
            print(f"{gain} point(s)! That's a record gain for Player {who}!")
            new_running = gain
        else:
            new_running = running_high
        
        return announce_highest(who, current, new_running)
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    pig = picky_piggy(opponent_score)
    if score + pig == opponent_score:
        return 0
    return picky_piggy_strategy(score, opponent_score, cutoff, num_rolls)


def final_strategy(score, opponent_score):
    """Risk-aware blend of Picky Piggy and Hog Pile.
    - Roll 0 if it immediately wins after adding Picky Piggy points
      or if it triggers Hog Pile (scores become equal).
    - Otherwise, roll 0 only when Picky Piggy >= a dynamic cutoff
      (lower cutoff when ahead, higher when behind).
    - Near the goal, use fewer dice; when ahead, play safer; when behind, roll more.
    - Fall back to a small set of fixed roll counts (e.g., 3/4/5/6) based on game state.
    """
    goal = GOAL_SCORE
    pig = picky_piggy(opponent_score)
    
    if score + pig >= goal or score + pig == opponent_score:
        return 0

    diff = score - opponent_score
    cutoff = 6 if diff >= 15 else (8 if diff <= -10 else 7)
